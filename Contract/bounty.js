...
interface MMR {
	...
	function verifyOnChain(uint blockNumber, bytes32 blockHash, bytes32[] calldata proof, uint localBlockNumber, bytes32[8] calldata localPeaks) external returns (bool, string memory);
}
...
contract Bounty {
	...
	function challenge(uint blockNumber, bytes32 blockHash1, bytes32[] memory proof1, uint localBlockNumber1, bytes32[8] memory localPeaks1, bytes32 blockHash2, bytes32[] memory proof2, uint localBlockNumber2, bytes32[8] memory localPeaks2) internal {
		...
		(bool isVerifed1,) = _mmr.verifyOnChain(blockNumber, blockHash1, proof1, localBlockNumber1, localPeaks1);
		require(isVerifed1, "blockHash1 not verified");
		(bool isVerifed2,) = _mmr.verifyOnChain(blockNumber, blockHash2, proof2, localBlockNumber2, localPeaks2);
